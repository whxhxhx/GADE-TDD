...
class DEM(nn.Module):
    def __init__(self, max_des_length=512, max_doc_length=256, device=0):
        super(DEM, self).__init__()
        if not isinstance(device, list):
            device = [device]
        # self.device = torch.device("cuda:{:d}".format(device[0]))
        self.device = torch.device("cuda")
        self.max_des_length = max_des_length
    ...
    def encode_feature(self, cand_docs):
        ...
        segment_ids = []
        input_masks = []
        des_tokens = ["[CLS]"] + cand_docs["description_token"] + ["[SEP]"]
        des_tokens = self.tokenizer.convert_tokens_to_ids(des_tokens)
        _, des_output = self.model(input_ids=des_tokens)
        ...
        for s in cand_docs["input_tokens"]:
            ...
            mask = [1] * len(tokens)
            padding = [0] * (self.max_doc_length - len(tokens))
            tokens += padding
        ...
        return pooled_output, des_output

    def document_interaction_graph_construction(self, cand_docs, max_n):
        features, des_feat = self.encode_feature(cand_docs)
        ...
        features = self.fusion(torch.cat([features, des_feat.unsqueeze(0).repeat(N, 1)], dim=1))
